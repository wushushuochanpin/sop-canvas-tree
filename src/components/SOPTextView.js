...
const LEVEL_COLORS = [
  "#1677ff", "#52c41a", "#faad14", "#722ed1", "#eb2f96", "#13c2c2",
];
...
const SOPTextView = ({ 
  ...
  return (
    ...
    >
      <div style={{ 
        width: "100%", maxWidth: "800px", marginBottom: 12, 
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 16 }}>
            <AntText type="secondary" style={{ fontSize: 11, fontFamily: "monospace" }}>
              ID: {flowMeta?.id}
            </AntText>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {visibleContentNodes.map((node) => {
            let jumpTargetCode = null;
            if (node.data.jumpTargetId) {
                const target = nodes.find(n => n.id === node.data.jumpTargetId);
                if (target) jumpTargetCode = target.computedCode;
            }

            const level = node.computedCode ? node.computedCode.split(".").length - 1 : 0;
            ...
            const codeColor = enableIndent ? "#1890ff" : LEVEL_COLORS[(level - 1) % LEVEL_COLORS.length] || "#1890ff";
            ...
            return (
              ...
              >
                {enableIndent && level > 0 && (
                  <div style={{ position: "absolute", left: -12, top: 0, bottom: 0, width: 1, background: "#f0f0f0" }} />
                )}
                ...
                <div style={{ flex: 1, paddingTop: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", marginBottom: 2, flexWrap: "wrap", gap: 6 }}>
                    <span 
                      style={{ 
                        fontSize: fontSize, 
                        fontWeight: 500, 
                    ...
                    {node.data.jumpTargetId && (
                      <Tag 
                        icon={<LinkOutlined />} 
                        color="default" 
                        style={{ fontSize: 10, margin: 0, padding: "0 6px", cursor: "default" }}
                      >
                        跳转至 {jumpTargetCode}
                      </Tag>
                    )}
        ...
        <div style={{ marginTop: 40, textAlign: "center", borderTop: "1px dashed #f0f0f0", paddingTop: 16 }}>
